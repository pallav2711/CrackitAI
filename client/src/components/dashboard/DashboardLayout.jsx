...
import {
  ...
  Menu, X, LogOut, Trophy, CreditCard, Settings,
...
const BASE_NAV = [
  ...
  { icon: FileText,        label: 'Resume Builder', path: '/resumes' },
  { icon: Briefcase,       label: 'JD Matcher',    path: '/jobs', badge: 'CORE', badgeColor: 'yellow' },
  { icon: HelpCircle,      label: 'Quiz',          path: '/jobs' },
  ...
  const NavLink = ({ item }) => {
    ...
    return (
      <Link
        ...
        className={`flex items-center gap-3 px-4 py-3 border-b-2 border-nb-black font-bold text-sm uppercase tracking-wide transition-colors duration-75 relative ${
          active
      ...
      >
        {active && <div className="absolute left-0 top-0 bottom-0 w-1 bg-nb-black" aria-hidden="true" />}
        <item.icon className="w-4 h-4 flex-shrink-0" aria-hidden="true" />
        <span className="flex-1 truncate">{item.label}</span>
        {item.badge && (
          <span
            className={`text-[9px] font-black px-1.5 py-0.5 border-2 border-nb-black leading-none ${
              item.badgeColor === 'red' ? 'bg-nb-red text-white' : 'bg-nb-black text-nb-yellow'
            }`}
        ...
        {active && <ChevronRight className="w-3.5 h-3.5 flex-shrink-0 opacity-60" aria-hidden="true" />}
      </Link>
    );
  };
    ...
    <div className="min-h-screen bg-[#F5F1E8]">
      ...
      >
        ...
        {user?.subscription?.plan && user.subscription.plan !== 'free' && (
          <span className="hidden sm:flex items-center text-[9px] font-black uppercase tracking-[0.2em] bg-nb-yellow text-nb-black px-2.5 py-1 border-2 border-nb-yellow" style={{ borderRadius: '3px' }}>
        ...
        <div className="flex items-center gap-2.5">
          <div className="flex items-center gap-2 border-2 border-nb-yellow/40 hover:border-nb-yellow px-3 py-1.5 transition-colors cursor-default" style={{ borderRadius: '5px' }}>
            <div className="w-6 h-6 bg-nb-yellow text-nb-black text-[11px] font-black flex items-center justify-center flex-shrink-0" style={{ borderRadius: '3px' }}>
              {initials}
            </div>
          ...
          >
            ...
            <span className="hidden md:inline">Match Job</span>
          </Link>
      ...
      <aside
        id="sidebar"
        className={`fixed top-16 left-0 z-40 h-[calc(100vh-4rem)] w-64 bg-white border-r-3 border-nb-black flex flex-col transition-transform duration-200 ease-out ${
          open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
        aria-label="Sidebar navigation"
      >
        <nav className="flex-1 overflow-y-auto border-t-2 border-nb-black" aria-label="Main">
          {NAV.map(item => <NavLink key={`${item.path}-${item.label}`} item={item} />)}
        </nav>
      ...
      {open && (
        <div className="fixed inset-0 z-30 bg-nb-black/60 lg:hidden" onClick={() => setOpen(false)} aria-hidden="true" />
      )}
